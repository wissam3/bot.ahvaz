...
local function check_member_super(cb_extra, success, result)
  ...
  if success == 0 then
	send_large_msg(receiver, "👌🏻موشغلڪ ابني لاتلعب😒✔️")
  end
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '✔️ تم تفعيل المجموعه يلحبيب😍 ✔️.'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '✔️ تم تعطيل المجموعه😕 ✔️.'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function promote2(receiver, member_username, user_id)
  ...
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' رفَعـوَكَ أدمـن 😉🎓 شدَ حيـلكَ ياشاطر💪🔅.')
end
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'رفع ادمن' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "👌🏻مٌوشغٌلّڪ آبنيِ فقَطَ المدير او الاداري يحَق لهَ✔️"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'رفع ادمن',
		...
		if matches[1] == 'مسح' then
			...
			if not is_momod(msg) then
				return "👌🏻مٌوشغٌلّڪ آبنيِ فقَطَ المدير  يحَق لهَ✔️"
			end
